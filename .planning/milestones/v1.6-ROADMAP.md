...
### Phase 9: Post-v1.6 cleanup

**Goal**: Resolve outstanding todos: config HTTP endpoint, httptest mock ServeMux routing, Windows header matching fix
**Depends on**: Phase 8
**Requirements**: (cleanup — no new requirements)
**Success Criteria** (what must be TRUE):

  1. Config provider exposes an HTTP handler for configuration updates
  2. httptest_mock uses http.ServeMux for mock request routing
  3. httptest_mock header matching works on Windows

**Plans**: 1 plan

Plans:

- [ ] 09-01-PLAN.md — Config HTTP endpoint, ServeMux routing, Windows header fix
